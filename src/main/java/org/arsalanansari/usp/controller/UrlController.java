...
public class UrlController {
    ...
    @GetMapping(value="{shorturl}",headers= "Host=dev.local:8991")
    ...
    public  ResponseEntity<?>  getshortUrl(@RequestBody String longurl) {
        ...
        if(url!=null && !url.isBlank()){
            return ResponseEntity.ok().body("http://dev.local:8991/"+url);
        }   
    ...
    public ResponseEntity<?> redirect2(@PathVariable String shorturl) {
        return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
}
